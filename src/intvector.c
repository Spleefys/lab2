...
int get_item(const Intvector *vect, int index) { return vect->p[index]; }

void set_item(Intvector *vect, int index, int item) { vect->p[index] = item; }
...
void clean_up(Intvector *v) { free(v); }

int get_size(const Intvector *v) { return v->size; }

int get_capacity(const Intvector *v) { return v->capacity; }
